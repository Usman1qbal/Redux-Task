...
class LowerSection extends Component {
  ...
  weekDetail = list => {
    ...
    let today = R.filter(list => {
      let date = new Date(list.dt_txt);
      let hour = date.getHours();
      return R.equals(hour,currentHour) && R.equals(date.getDay(),currentDay);
    }, list);

    let week = R.filter(list => {
      let date = new Date(list.dt_txt);
      let hour = date.getHours();
      return R.equals(hour,12) && !R.equals(date.getDay(),currentDay);
    }, list);
  ...
  graph = () => {
    this.setState({
      chart: new Highcharts.chart("chartsContainer", {
        chart: {
          events: {
            redraw: function() {
  ...
  SetTempFarenhiet = () => {
    if (!R.equals(this.state.degree,"F")) {
      let weekArray = R.map(item => {
      ...
      let updateTotalData = R.map(item => {
        item.temperature = this.celciusToFarenhietFormula(item.temperature);
        item.minTemp = this.celciusToFarenhietFormula(item.minTemp);
        item.maxTemp = this.celciusToFarenhietFormula(item.maxTemp);
        return item;
      ...
      let temp = R.map(item => {
        return item = this.celciusToFarenhietFormula(item);
      }, this.state.tempList);
  ...
  setData = value => {
    if (!R.equals(value.length,0) && !R.equals(value.cod,"404")) {
      const weekData = this.weekDetail(value.list);

      let totalDetail = R.map(item => {
  ...
  dayNameFind = id => {
    if (!R.equals(id,undefined)) {
      let dat = new Date(id);
      let dayID = dat.getDay();
      let today = R.filter(dayList => {
        return R.equals(dayList.id,dayID);
      }, this.state.dayList);
  ...
  render() {
    return (
      <div>
        {R.equals(this.state.check,true) ? (
            ...
            <section className="lowerSection">
              ...
              <div className="middleGrid">
                <div className="middleImgDiv">
                  {" "}
                  <img src={this.state.imgSrc} alt="Today Weather" />
                </div>
                ...
                <div className="temperatureType">
                  <a id="celciusBtn" onClick={this.SetTempCelcius} ref="celcius" >
                    {" "}ºC{" "}</a>{" "}
                  <label> | </label>
                  <a id="farenhietBtn" onClick={this.SetTempFarenhiet} ref="farenhiet" >
                    {" "}ºF{" "}
                  </a>
                </div>
                ...
                <div className="showDetail">
                  <p>
                    Pressure:{" "}
                    <label id="pressure">{this.state.pressure} </label> hPa{" "}
                  </p>
                  <p>
                    Humidity:{" "}
                    <label id="humidity">{this.state.humidity} </label> %{" "}
                  </p>
                  <p>
                    Wind Speed: <label id="wind">{this.state.windSpeed} </label>{" "}
                    mph{" "}
                  </p>
                </div>
              </div>
